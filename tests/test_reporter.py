...
def test_write_report_for_pipeline_result(tmp_path: Path) -> None:
    ...
    assert set(payload["localized_texts"]) == {"en", "ja"}
    assert payload["validation_results"]["product_count"] == 3
    assert payload["validation_results"]["final_creatives_rendered"] == 24
    assert payload["products"][0]["variants"][0]["rendition_paths"]
    assert set(payload["products"][0]["variants"][0]["rendition_paths_by_locale"]) == {"en", "ja"}
